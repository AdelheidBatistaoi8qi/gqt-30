...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-17a9052d";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-17a9052d">'+'<span class="tile-face-17a9052d tile-front-17a9052d"></span>'+'<span class="tile-face-17a9052d tile-back-17a9052d"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-17a9052d")){return;}
var back=tile.querySelector(".tile-back-17a9052d");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-17a9052d");if(isMine){tile.classList.add("boom-17a9052d");}
if(dim){tile.classList.add("dimmed-17a9052d");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-17a9052d")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-17a9052d");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-17a9052d")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-17a9052d");void panelEl.offsetWidth;panelEl.classList.add("jolt-17a9052d");}
endRound();}
